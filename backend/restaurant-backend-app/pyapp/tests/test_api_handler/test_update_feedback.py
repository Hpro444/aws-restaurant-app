"""Tests for the PUT /feedbacks endpoint."""
...
class TestUpdateFeedback(ApiHandlerLambdaTestCase):
    """Tests for customer feedback update endpoint."""
    ...
    def setUp(self) -> None:
        ...
        self.HANDLER._feedback_service.update_feedback = MagicMock()

    def test_success_returns_200_with_message(self) -> None:
        """A valid customer request returns updated status with success message."""
        event = make_event(_PATH, "PUT", body=_VALID_BODY, headers=_VALID_HEADERS)

        result = self.HANDLER.lambda_handler(event, {})

        self.assertEqual(status(result), 200)
        self.assertEqual(body(result)["message"], "Feedback has been updated.")
        self.HANDLER._feedback_service.update_feedback.assert_called_once()
        kwargs = self.HANDLER._feedback_service.update_feedback.call_args.kwargs
        ...
        self.assertEqual(kwargs["request"].rating, 4)
    ...
    def test_missing_authorization_header_returns_401(self) -> None:
        ...
        result = self.HANDLER.lambda_handler(
            make_event(_PATH, "PUT", body=_VALID_BODY), {}
        )

        self.assertEqual(status(result), 401)
        self.HANDLER._feedback_service.update_feedback.assert_not_called()

    def test_non_customer_role_returns_403(self) -> None:
        """Only customers are allowed to edit feedback."""
        self.HANDLER._cognito_service.get_identity_from_access_token = MagicMock(
            return_value=(str(uuid4()), UserRole.WAITER)
        )

        result = self.HANDLER.lambda_handler(
            make_event(_PATH, "PUT", body=_VALID_BODY, headers=_VALID_HEADERS),
            {},
        )

        self.assertEqual(status(result), 403)
        self.assertEqual(body(result)["message"], "Only customers can edit feedback.")
        self.HANDLER._feedback_service.update_feedback.assert_not_called()
    ...
    def test_missing_reservation_id_returns_422(self) -> None:
        ...
        result = self.HANDLER.lambda_handler(
            make_event(_PATH, "PUT", body=bad_body, headers=_VALID_HEADERS),
            {},
        )
    ...
    def test_service_not_found_error_returns_404(self) -> None:
        ...
        self.HANDLER._feedback_service.update_feedback = MagicMock(
            side_effect=ApplicationException(
                404, "Service feedback for this reservation was not found."
            )
        )
